...
#
# This script validates that @qwickapps/react-framework can be installed
# and used in a completely clean environment (fresh React project).
#
# Usage:
...
SCHEMA_DIR="$(cd "$PACKAGE_DIR/../qwickapps-schema" && pwd)"

echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Clean Environment Validation Test                             ║"
echo "║  @qwickapps/react-framework                                    ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
echo "📦 Step 1/4: Building @qwickapps/schema..."
cd "$SCHEMA_DIR"
...
echo ""
echo "📦 Step 2/4: Building @qwickapps/react-framework..."
cd "$PACKAGE_DIR"
npm run build > /dev/null 2>&1
npm pack --pack-destination "$BUILD_CONTEXT/packages" > /dev/null 2>&1
echo "   ✓ React framework package built"

echo ""
echo "🐳 Step 3/4: Preparing Docker build context..."
cp "$SCRIPT_DIR/Dockerfile" "$BUILD_CONTEXT/"
cp "$SCRIPT_DIR/test-app.tsx" "$BUILD_CONTEXT/"
echo "   ✓ Build context ready"

echo ""
echo "🔨 Step 4/4: Running Docker validation..."
...
BUILD_OUTPUT=$(docker build -t qwickapps-validation-test . 2>&1)
BUILD_EXIT_CODE=$?
...
if [ $BUILD_EXIT_CODE -eq 0 ]; then
    ...
    docker rmi qwickapps-validation-test > /dev/null 2>&1 || true
    exit 0
else
    echo ""
    echo "Build errors:"
